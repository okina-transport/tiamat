...
public class StopPlaceRepositoryImpl implements StopPlaceRepositoryCustom {
    ...
    public List<String> getAllProviders() {
        String sql = "SELECT DISTINCT s.provider FROM stop_place s WHERE s.provider IS NOT NULL ORDER BY s.provider";
    ...
    @Override
    public StopPlace findFirstStopPlaceByNetexQuayOrderByVersionDesc(String quayNetexId){
        final String queryString = "SELECT sp.* FROM stop_place sp " +
                "JOIN stop_place_quays spq on sp.id = spq.stop_place_id  " +
                "JOIN quay q on q.id = spq.quays_id  " +
                "WHERE q.netex_id = :quayNetexId " +
                "AND q.version = (SELECT max(q2.version) FROM quay q2 WHERE q2.netex_id = q.netex_id)";

        List<StopPlace> results = entityManager.createNativeQuery(queryString, StopPlace.class)
                .setParameter("quayNetexId",quayNetexId)
                .getResultList();
        results.forEach(this::initializeStopPlace);
        return results.isEmpty() ? null : results.get(0);
    }
