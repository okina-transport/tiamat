...
for l3 in (select qa.alternative_names_id qaid
    from quay_alternative_names qa
    where qa.quay_id = l2.qid
)
loop
delete from quay_alternative_names where quay_id = l2.qid;
delete from alternative_name an where an.id = l3.qaid;
end loop;
...
delete from stop_place_alternative_texts where stop_place_id = l.spid;
delete from stop_place_tariff_zones      where stop_place_id = l.spid;
delete from stop_place_other_transport_modes where stop_place_id = l.spid;
delete from stop_place_equipment_places  where stop_place_id = l.spid;
delete from stop_place_access_spaces     where stop_place_id = l.spid;
...
for l2 in (select qa.alternative_names_id qaid
    from quay_alternative_names qa
    where qa.quay_id = l.qid
)
loop
delete from quay_alternative_names where quay_id = l.qid;
delete from alternative_name an where an.id = l2.qaid;
end loop;
