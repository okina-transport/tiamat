...
---------------------------------------------------
-- DETECTION OF IMPORTED-ID DUPLICATES FOR QUAYS
---------------------------------------------------
...
---------------------------------------------------
-- DETECTION OF IMPORTED-ID DUPLICATES FOR STOP PLACES
---------------------------------------------------

FOR imported_id_record IN (SELECT DISTINCT vi.items AS items FROM (
					 SELECT DISTINCT vi_orig.items, vi_orig.value_id FROM value_items vi_orig
					  						   INNER JOIN stop_place_key_values spkv_orig  ON vi_orig.value_id = spkv_orig.key_values_id
					     						   WHERE spkv_orig.key_values_key ='imported-id'
	  								) vi
  					INNER JOIN stop_place_key_values spkv  ON  vi.value_id = spkv.key_values_id
  					INNER JOIN stop_place sp ON  sp.id = spkv.stop_place_id
  							WHERE spkv.key_values_key ='imported-id' AND sp.from_date < now()
  									AND (sp.to_date IS NULL OR sp.to_date > now())  GROUP BY vi.items HAVING count(*) > 1
  							)
  		LOOP

  		FOR netex_id_record IN (SELECT DISTINCT sp.netex_id AS sp_netex_id FROM stop_place sp INNER JOIN stop_place_key_values spkv  ON  sp.id = spkv.stop_place_id
		...
		LOOP
			...
			INSERT INTO tmp_duplicate_imported_ids ( parent_netex_id,imported_id)
			VALUES (netex_id_record.sp_netex_id, imported_id_record.items);
