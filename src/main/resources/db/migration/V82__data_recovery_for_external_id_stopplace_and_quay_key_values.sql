...
FROM public.stop_place sp
    JOIN public.stop_place_key_values spkv ON sp.id = spkv.stop_place_id
    JOIN public.value v ON spkv.key_values_id = v.id
    JOIN public.value_items vi ON v.id = vi.value_id
WHERE spkv.key_values_key = 'imported-id'
  AND vi.items IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.stop_place_key_values spkv_check
...
SELECT DISTINCT ON (q.id)
    ...
    replace(vi.items, '##3A##', ':') AS imported_id_value
FROM public.quay q
    JOIN public.quay_key_values qkv ON q.id = qkv.quay_id
    JOIN public.value v ON qkv.key_values_id = v.id
    JOIN public.value_items vi ON v.id = vi.value_id
WHERE qkv.key_values_key = 'imported-id'
  AND vi.items IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.quay_key_values qkv_check
...
SELECT GREATEST(
               COALESCE(MAX(id), 0),
               (SELECT COALESCE(MAX(key_values_id), 0) FROM public.stop_place_key_values),
               (SELECT COALESCE(MAX(key_values_id), 0) FROM public.quay_key_values)
       ) + 1 INTO v_max_seq FROM public.value;

EXECUTE format('ALTER SEQUENCE hibernate_sequence RESTART WITH %s', v_max_seq);
